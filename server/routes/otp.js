...
function buildHtml(d) {
  ...
  const addendumItems = Array.isArray(d.addendumItems)
    ? d.addendumItems.filter(it => it && (it.title || it.body))
    : [];
  const hasAddendum = addendumItems.length > 0;
  const totalPages = hasAddendum ? 5 : 4;
  const pageOf = (n) => `Page ${n} of ${totalPages}`;
...
<div class="page page-break">
  ...
  <div style="text-align:center; margin-top:6px; font-size:7pt; color:#aaa;">
    This document was generated by the Dealmaker Portal · Peterson Acquisitions · petersonacquisitions.com
  </div>
...
<div class="page page-break">
  ...
  <!-- Compact header -->
  <div style="display:flex; align-items:center; gap:10px; border-bottom:1.5px solid #1a3a6b; padding-bottom:7px; margin-bottom:14px;">
    ${logo ? `<img style="width:32px;height:32px;object-fit:contain;" src="${logo}" alt="" />` : ''}
    <div style="font-family:Arial,sans-serif; font-weight:900; font-size:10.5pt; color:#1a3a6b; letter-spacing:1.5px; text-transform:uppercase;">Peterson Acquisitions — Offer to Purchase</div>
    <div style="margin-left:auto; font-size:8.5pt; color:#555;">${blank(d.businessName)}</div>
  </div>

  <div class="release-title" style="text-align:center; margin-bottom:4px;">Addendum to Offer to Purchase</div>
  <div style="text-align:center; font-size:9pt; color:#555; font-style:italic; margin-bottom:16px;">
    This Addendum is incorporated into and made a part of the Offer to Purchase dated ${d.date || '___________'} for the business known as ${d.businessName || '___________'}.
  </div>

  ${addendumItems.map((item, idx) => `
  <div class="section" style="margin-bottom:16px;">
    <div class="section-title">${idx + 1}. ${item.title ? item.title.replace(/</g, '&lt;').replace(/>/g, '&gt;') : 'Additional Term'}</div>
    <p style="white-space:pre-wrap;">${(item.body || '').replace(/</g, '&lt;').replace(/>/g, '&gt;')}</p>
  </div>
  `).join('')}

  <!-- Addendum Signature Block -->
  <div style="margin-top:20px; border-top:1px solid #ccc; padding-top:14px;">
    <div class="section-title" style="margin-bottom:10px;">Acknowledgment of Addendum</div>
    <p style="font-size:9.5pt; font-style:italic; margin-bottom:12px;">
      By signing below, both Buyer and Seller acknowledge and agree that this Addendum is incorporated into and forms an integral part of the Offer to Purchase referenced above, and that all terms herein are binding to the same extent as the terms of the Offer to Purchase itself.
    </p>
    <div class="sig-grid">
      <div class="sig-field">
        <div class="sig-line">${d.buyerName || ''}</div>
        <div class="sig-label">Buyer's Printed Name</div>
      </div>
      <div class="sig-field">
        <div class="sig-line" style="min-height:22px;">&nbsp;</div>
        <div class="sig-label">Buyer's Signature</div>
      ...
      <div class="sig-field">
        <div class="sig-line">${d.sellerName || ''}</div>
        <div class="sig-label">Seller's Printed Name</div>
      </div>
      <div class="sig-field">
        <div class="sig-line" style="min-height:22px;">&nbsp;</div>
        <div class="sig-label">Seller's Signature</div>
  ...
  <div class="doc-footer">
    <span>Peterson Acquisitions — Offer to Purchase Addendum</span>
    <span>${pageOf(5)}</span>
  </div>
