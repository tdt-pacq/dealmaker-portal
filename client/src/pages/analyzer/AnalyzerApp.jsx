...
const isYtdYear = y => String(y?.year) === 'YTD';
const yearNum = y => {
...
const sortedByYear = yrs => [...yrs].map((y,i)=>({y,i})).sort((a,b)=>{
...
const ytdMonthsFromStr = through => {
  if (!through) return 0;
  const m = Number(through.split('-')[1]);
  return (m >= 1 && m <= 11) ? m : 0;
};
// Scale all numeric fields of a year-data object by (12 / months)
const annualizeYTD = (yd, months) => {
  if (!months) return yd;
  const f = 12 / months;
  const sc = v => String(Math.round(pn(v) * f));
  return {
    ...yd,
    revenue: sc(yd.revenue), cogs: sc(yd.cogs), opx: sc(yd.opx),
    otherIncome: sc(yd.otherIncome||0), interest: sc(yd.interest),
    taxes: sc(yd.taxes), depreciation: sc(yd.depreciation),
    amortization: sc(yd.amortization), ownerComp: sc(yd.ownerComp),
    addBacks: (yd.addBacks||[]).map(ab=>({...ab, amount: sc(ab.amount)})),
    rent: sc(yd.rent||0), rentAdj: sc(yd.rentAdj||0),
  };
};
// Returns annualized SDE from ytdData + through string, or 0 if not computable
const annYTDSDE = (ytdData, ytdThrough) => {
  const m = ytdMonthsFromStr(ytdThrough);
...
const FinancialSpreadTable = ({years, ytdThrough=''}) => {
  const months = ytdMonthsFromStr(ytdThrough);
  return (
        ...
        return (
          <tr key={lbl} style={{borderBottom:`1px solid ${bold?'#1e2d45':'#0d1117'}`,background:isSDE?'#061208':bold?'#0a1205':'transparent'}}>
            ...
            {years.map((y,i)=>{
              ...
              const va = hasAnn ? fn(annYd) : null;
              const annRev = hasAnn ? calcSDE(annYd).rev : 0;
      ...
      <div className="flex items-center justify-between px-5 py-4 cursor-pointer select-none hover:bg-gray-900 rounded-lg"
        ...
        <div className="flex items-center gap-4">
          ...
          {String(yd.year)==='YTD'?(
            ...
            <input className="input-field py-1.5 font-bold text-blue-300" style={{fontSize:15,width:76,textAlign:'center',fontWeight:700}}
              value={yd.year} onClick={e=>e.stopPropagation()}
              onChange={e=>{
...
const T1 = ({state,set,primeRate,importTaxReturn}) => {
  ...
  const syncSection=(idx, newYd)=>{
    ...
    if(state.ytdEnabled){
      ...
      set({...state,years,ytdData:next[next.length-1]});
    } else {
    ...
    <div>
      ...
      <div className="report-body">
        ...
        {vis.roi&&(()=>{
          ...
          const rsde=resolveSDE(state).basis;
...
const TAcqCalc = ({state}) => {
  const mpaSDE = resolveSDE(state).basis;
...
function App() {
  ...
  const [state,setState]=useState(()=>{
    ...
    const merged=draft?{...initState(),...draft,_net:0}:initState();
    return merged.years?.length?{...merged,years:uniquifyYears(merged.years)}:merged;
  });
  ...
  const migrateYears=d=>{if(d?.years?.length){d={...d,years:uniquifyYears(d.years)};}return d;};
  const migrateDeal=d=>migrateBuyerSalary(migrateYears(migrateBs(d)));
  const load=data=>{data=migrateDeal(data);setState({...initState(),...data,_net:0});setShowLoad(false);setTab('dashboard');};
  const newDeal=()=>{
  ...
  const importDeal=()=>{
    ...
    input.onchange=e=>{
      ...
      reader.onload=ev=>{
        try{let data=JSON.parse(ev.target.result);data=migrateDeal(data);setState({...initState(),...data,_net:0});setTab('dashboard');}
  ...
  const applyReview=confirmed=>{
    const applyIncome=(prev,inc,yi)=>{
      ...
      years[yi]={...y,
        ...
        year:inc.year!=null&&inc.year!==''?(Number(inc.year)||y.year):y.year,
